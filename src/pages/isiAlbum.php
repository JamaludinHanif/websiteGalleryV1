<?php
    ...
    $namaAlbum = $_GET['namaalbum'];

    $sqlAlbum = "SELECT * FROM tbl_album WHERE AlbumId = $albumId";
    $queryAlbum = mysqli_query($koneksi, $sqlAlbum);
    $dataAlbum = mysqli_fetch_array($queryAlbum);
...
<div class="bg-white">
  ...
  <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
    <p class="font-bold text-xl mt-5"><?= $dataAlbum['NamaAlbum']; ?></p>
    <p class="mt-2 w-2/3"><?= $dataAlbum['Deskripsi']; ?></p>
    <p class="mt-2 text-blue-700"><?= $dataAlbum['TanggalDibuat']; ?></p>
    <div class="mt-8 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8">
    <?php                            
        // select data
        $sql = "SELECT * FROM tbl_foto WHERE AlbumId = $albumId";
    ...
    ?>
      <div>
        <div class="relative">
          <div class="relative mt-2 h-72 w-full overflow-hidden rounded-lg">
